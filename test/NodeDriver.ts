...
describe('NodeDriver', () => {
    ...
    describe('Migrate', () => {
        ...
        it('Should revert when not owner', async function () {
            const account = ethers.Wallet.createRandom()
            await expect(this.nodeDriverAuth.connect(this.nonOwner).migrateTo(account))
    ...
    describe('Copy code', () => {
        ...
        it('Should revert when not owner', async function () {
            ...
            await expect(this.nodeDriverAuth.connect(this.nonOwner).copyCode(this.sfc, address))
                .to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe('Update network version', () => {
        ...
        it('Should revert when not owner', async function () {
            await expect(this.nodeDriverAuth.connect(this.nonOwner).updateNetworkVersion(1))
                .to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe('Advance epoch', () => {
        it('Should succeed and advance epoch', async function () {
            await expect(this.nodeDriverAuth.advanceEpochs(10))
                .to.emit(this.nodeDriver, 'AdvanceEpochs')
                .withArgs(10);
        });

        it('Should revert when not owner', async function () {
            await expect(this.nodeDriverAuth.connect(this.nonOwner).advanceEpochs(10))
                .to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe('Set storage', () => {
        it('Should revert when not backend', async function () {
            ...
            await expect(this.nodeDriver.setStorage(account, key, value))
    ...
    describe('Set backend', () => {
        it('Should revert when not backend', async function () {
            const account = ethers.Wallet.createRandom();
            await expect(this.nodeDriver.setBackend(account))
                .to.be.revertedWith("caller is not the backend");
        });
    });
    ...
    describe('Swap code', () => {
        it('Should revert when not backend', async function () {
            ...
            await expect(this.nodeDriver.swapCode(account, account2))
                .to.be.revertedWith("caller is not the backend");
        });
    });
    ...
    describe('Add genesis validator', () => {
        it('Should revert when not node', async function () {
            const account = ethers.Wallet.createRandom();
            await expect(this.nodeDriver.setGenesisValidator(account, 1, account.publicKey, 0, await this.sfc.currentEpoch(), Date.now(), 0, 0))
    ...
    describe('Set genesis delegation', () => {
        it('Should revert when not node', async function () {
            const account = ethers.Wallet.createRandom();
            await expect(this.nodeDriver.setGenesisDelegation(account, 1, 100, 0, 0, 0, 0, 0, 1000))
                .to.be.revertedWith("not callable");
        });
    });
